  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hey! I'm Aura—your gentle cycle buddy. How can I support you today?",
    },
  ]);
  ...
  const hint = useMemo(
    () => (
      <div className="text-xs text-muted-foreground">
        Try: "cramps relief", "when is my next period?", "low energy today"
      </div>
    ),
    [],
  );
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button
          id="chatbot"
          size="lg"
          className="fixed bottom-6 right-6 rounded-full shadow-lg bg-gradient-to-br from-rose-500 via-fuchsia-500 to-violet-500 text-white hover:opacity-90"
        >
      ...
      <DrawerContent className="max-h-[80vh]">
        <DrawerHeader>
          <DrawerTitle className="bg-clip-text text-transparent bg-gradient-to-r from-rose-500 via-fuchsia-500 to-violet-500">
            Aura Chat
          </DrawerTitle>
          {hint}
        </DrawerHeader>
        ...
        <DrawerFooter>
          <div className="flex items-end gap-2">
            ...
            />
            <Button onClick={handleSend} className="h-12 px-4">
